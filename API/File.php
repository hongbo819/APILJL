...
class API_File
{
	...
	public static function exists($file)
	{
		...
		$abs = ($file[0] == '/' || $file[0] == '\\' || $file[1] == ':' || $file[0]=='.');
	...
	*/
	...
	public static function load($file)
	{
		...
		{
			throw new LJL_Exception('File does not readable: ' . $file);
		}
	...
	public static function copyDir($source, $dest, $overwrite = false)
	{
		if (!is_dir($dest)) {
			if (!is_writable(dirname($dest))) {
				throw new LJL_Exception('filesystem not writable:' . dirname($dest));
			}
			mkdir($dest);
		}
					...
					if (self::exists($path)) {
						if (!self::exists($dest . '/' . $file) || $overwrite) {
							if (!@copy($path, $dest . '/' . $file)){
								throw new LJL_Exception('filesystem not writable:' . $dest . '/' . $file);
							}
						}
					} elseif (is_dir($path)) {
						if (!is_dir($dest . '/' . $file)) {
							if (!is_writable(dirname($dest . '/' . $file))) {
								throw new LJL_Exception('filesystem not writable:' . dirname($dest . '/' . $file));
							}
