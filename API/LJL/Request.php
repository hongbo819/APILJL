...
class LJL_Request
{
	...
	public function __construct()
	{
		if ($this->isEmpty()) 
		{
			...
			$this->setType($type);
			if ($type == LJL_Request::CLI)
			{
				$this->_a['get'] = $this->getCliOpt();
			}
			elseif ($type == LJL_Request::BROWSER || $type == LJL_Request::AJAX)
			{
				$this->_a['get'] = $_GET;
				$this->_a['post'] = $_POST;
	...
	*/
	...
	public function __get($key)
	{
		...
		{
			//throw new LJL_Exception("Notice: Undefined variable '$key'");
	...
	*/
	...
	public function getZhbUserId()
	{
		$ret = '';
		if (!isset($this->_aClean['zhb_userid']))
		{
			if (true === $this->checkOutUserId($this->cookie('zhb_userid'), $this->cookie('zhb_check'),
				$this->cookie('zhb_cipher')))
			{
				$ret = $this->cookie('zhb_userid');
				$this->set('zhb_userid', $ret);
			}
			
		}
		else
		{
			$ret = $this->_aClean['zhb_userid'];
		}
		return $ret;
	}
	...
	*/
	protected function checkOutUserId($userid, $checkid, $cipher)
	{
		...
		$key = "sa^2fa*%mdpyw$@4";
		$zhb_cipher = md5(md5($key . $checkid) . $userid . $key);
		if ($zhb_cipher == $cipher)
		{
			return true;
		}
		else
