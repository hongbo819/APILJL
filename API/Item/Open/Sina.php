...
{
    private static $WB_AKEY = "3206794236";
    private static $WB_SKEY = "ef80185c134f9a83a8129db19b98edc2";
    ...
    public static function getLoginTokens($paramArr){
        ...
        if (is_array($paramArr))$options = array_merge($options, $paramArr);
        extract($options);
        if(!$code || !$redirect_uri) return false;
    
        $loginObj = new SaeTOAuthV2(self::$WB_AKEY, self::$WB_SKEY);
        $tokens = $loginObj->getAccessToken('code', array(
            'code' => $code,
            'redirect_uri' => $redirect_uri,
            'refresh_token'   => ''
        ) );
    ...
    private static function init($paramArr) {
        ...
        if(!isset(self::$conObj[$accessToken])){
            ...
            self::$conObj[$accessToken] = new SaeTClientV2( self::$WB_AKEY , self::$WB_SKEY , $accessToken);
     ...
     */
     public static function getUserApiId($paramArr) {
        ...
        #获得API id
        return $db->getOne("select api_uid from api_user_map where app_uid = '{$appuid}' and api_type='sina'");
    }
    /**
     * 获得用户的api token
     */
    public static function getAccessToken($paramArr) {
        ...
        if (is_array($paramArr))$options = array_merge($options, $paramArr);
        extract($options);
        if(!$appuid && !$apiuid && !$name)return false;
    
        $db = API_Db_User::instance();
        $where = '';
        if($appuid){
            $where = " where app_uid='{$appuid}' ";
        }elseif($apiuid){
            $where = " where api_uid='{$apiuid}' ";
        }elseif($name){
            $where = " where api_name='{$name}' ";
        }
        #获得API id
        return $db->getOne("select api_token from api_user_map {$where} and api_type='sina'");
    }
    /**
    ...
    public static function getUserInfo($paramArr) {
        ...
        if(!$name && !$apiuid && $appuid)
            $apiuid = self::getUserApiId(array('appuid'=>$appuid));#app用户ID到新浪ID的转换
        if($apiuid){ 
            $info = $c->show_user_by_id($apiuid);
        }elseif($name){
    ...
    public static function getUserFriends($paramArr) {
		$options = array(
		    'appuid'         => 0,
			'apiuid'         => 0,  #用户ID 与 name必填一个
			'name'           => '', #用户名
			'offset'         => 0,  #游标值，返回 的next_cursor的值指下一页
			'num'            => 50,  #获得粉丝数
			'accessToken'    => '', #accessToken
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
        ...
        $info = false;
        if($apiuid){
            $info = $c->friends_by_id($apiuid, $offset, $num);
        }elseif($name){
            $info = $c->friends_by_name($name, $offset, $num);
        }
        return $info;
    ...
    public static function getUserFollowers($paramArr) {
        ...
        $info = false;
        if($apiuid){
            $info = $c->followers_by_id($apiuid,$offset,$num);
        }elseif($name){
    ...
    public static function getUserTimeLine($paramArr) {
        ...
        if(!$name && !$apiuid && $appuid)
            $apiuid = self::getUserApiId(array('appuid'=>$appuid));#app用户ID到新浪ID的转换
        if ($apiuid){
            $data = $c->user_timeline_by_id( $apiuid , $page , $num , $sinceId, $maxId, $feature, $trimUser, $baseApp);
        } elseif($name) {
            $data = $c->user_timeline_by_name($name , $page , $num , $sinceId, $maxId, $feature, $trimUser, $baseApp);
        } 
        //var_dump($data);die;
    ...
    public static function getPublicTimeLine($paramArr) {
		$options = array(
			...
			'page'           => 1,  #第几页，默认1
			'num'            => 50, #数量
			'baseApp'        => 0,  #是否只获取当前应用的数据。0为否（所有数据），1为是（仅当前应用），默认为0。
		);
		if (is_array($paramArr))$options = array_merge($options, $paramArr);
		extract($options);
    ...
    public static function getHomeTimeLine($paramArr) {
        ...
        $c = self::getWeiboObj(array('appuid' => $appuid,'apiuid' => $apiuid,'name' => $name,));
        if(!$c)return false;
    ...
    {
        $options = array(
            'url'    => '',   #源URL地址    记得带上#后的 http://cui.zhbor.com/article/13.html#0-tsina-1-38138-397232819ff9a47a7b7e80a40613cfe1
            'num'    => 10,   #返回的条数
        ...
        if ($list) {
            foreach ($list as $key=>$item) {
                $mids .= ($key==0) ? '' : ',';
                $mids .= $item['mid']; 
                $item['pic_urls'] = isset($item['pic_urls']) ? $item['pic_urls'] : '';
                $data[$key]['id']   = $item['idstr'];  #评论ID
                $data[$key]['text'] = $item['text']; #评论内容
                ...
                $data[$key]['nickname'] = $item['user']['screen_name']; #发表人昵称
    ...
    public static function getWeiboByTopics($paramArr)
    {
        ...
        if ($list) {
            foreach ($list as $key=>$item) {
                ...
                $data[$key]['text'] = $item['text']; #评论内容
                $data[$key]['time'] = strtotime($item['created_at']); #评论时间
                $data[$key]['picurl']   = $item['original_pic']; #内容配图地址
                $data[$key]['sid']      = $item['user']['id']; #发表人ID
                $data[$key]['nickname'] = $item['user']['screen_name']; #发表人昵称
                $data[$key]['headpic']  = $item['user']['profile_image_url']; #发送人头像 50
                $data[$key]['userurl']  = "http://weibo.com/".$item['user']['profile_url'];  #发送人链接
