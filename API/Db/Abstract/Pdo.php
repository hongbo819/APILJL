...
abstract class API_Db_Abstract_Pdo
{
	/**
	* 当前数据库链接
	* 
	...
	*/
	protected function createDbConn($dbType = 'master')
	{
		if (empty($this->$dbType)) {
			...
			try {
				$this->$dbType = new PDO($dns, $this->username, $this->password,array(
	...
	protected function chooseDbConn($sql)
	{
        ...
        if (empty($this->db)) {
            throw new API_LJL_Exception('Dose not exist instance of DB server!');
        }
	...
	public function getSqlComment(){  
            if(LJL_Api::$_nowMethod){#如果记录当前了当前的方法
                $this->sqlComment = "/* CLOUD API ".LJL_Api::$_nowMethod ." */";
            }else{
                if(!$this->sqlComment){          
                    $tmpStr = "";
                    if($_SERVER){
